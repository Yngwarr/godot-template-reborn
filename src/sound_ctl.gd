...
## In-editor sliders go to as far as 6, but I think default being max is more
## user-friendly.
...
static func get_volume(bus: int) -> float:
...
static func _showable_bus(bus_idx: int) -> bool:
	return (
		AudioServer.get_bus_name(bus_idx) == MASTER_NAME
